...
ff1.Cache.enable_cache('src/commands/plots/python/cache')  # replace with your cache directory
...
if lap_nb == 'Fastest':
    ...
    lap_nb = int(sys.argv[4+1])
    lap= lap[lap['LapNumber'] == lap_nb].iloc[0]
...
fig.suptitle(f'{weekend.name} {year} - {driver} - Speed\nF1 Telemetry Discord Bot - Developed by LapsTime, Flip and Walshaw', size=24, y=1)
...
norm = plt.Normalize(color.min(), color.max())  # type: ignore
lc = LineCollection(segments, cmap=colormap, norm=norm, linestyle='-', linewidth=5)  # type: ignore
...
legend = mpl.colorbar.ColorbarBase(cbaxes, norm=normlegend, cmap=colormap, orientation="horizontal")  # type: ignore
