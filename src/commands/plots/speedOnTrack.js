...
module.exports = {
	data: new SlashCommandBuilder()
		.setName('speedontrack')
		.setDescription('Create a comparative map of the speed on a lap')
	...
	async python(interaction) {
		...
		const python = spawn('python3', ['src/commands/plots/python/speedOnTrack.py', Year, GP, Session, Driver1, Lap]);
		...
		python.on('close', async (code) => {
			...
			await interaction.editReply({content: `Speed map ${Driver1} - ${GP} ${Year} ${Session}`, files: [attachment]});
		});
	}
};